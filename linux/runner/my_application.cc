...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "DBS Annotator");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "DBS Annotator");
  }

  // Modest default: this is the frame shown BEFORE Dart runs, and Dart then
  // fits the window to the display's work area (see lib/main.dart,
  // _fitWindowToWorkArea). A large default here opens off-screen on a small
